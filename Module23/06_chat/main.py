...
async def main():
    ...
    try:
        nickname = await input("Погрузиться в чат", required=True, placeholder="Введите никнейм ")
        nickname not in online_users
    except ValueError:
        nickname = await input("Погрузиться в чат", required=True, placeholder="Введите никнейм ",
                               validate=lambda n: "Ник уже занят! Придумай другой." if n in online_users or n == '📢' else None)
    ...
    while True:
        data = await input_group("💭 Выберите действие",[
